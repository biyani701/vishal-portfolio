...
test('an old article link lands on the new URL, and Back skips the old one', async ({ page }) => {
  await page.goto('/')
  await page.goto('/blogs/ai-agents')
  await expect(page).toHaveURL('/writing/ai-agents')
  await expect(page.getByRole('heading', { level: 1 })).toHaveText('Article')
