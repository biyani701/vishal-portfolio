...
const valid = { VITE_API_BASE_URL: 'https://api.example.test', VITE_AUTH_SERVER_URL: 'https://auth.example.test' }
...
describe('build-time configuration check', { timeout: 60_000 }, () => {
  it('fails the build naming a missing VITE_API_BASE_URL', async () => {
    await expect(buildWith({ VITE_AUTH_SERVER_URL: valid.VITE_AUTH_SERVER_URL })).rejects.toThrow(
