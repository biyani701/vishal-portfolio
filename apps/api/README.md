...
`api.vishal.biyani.xyz`. It exposes `GET /health` and the cron job `GET /cron/ai-models` (bearer
`CRON_SECRET`); `POST /contact` (task 10.3) and the Ask runtime (task 11.2) follow.
...
- **AI models:** discovered, not configured (`src/ai/`): the catalog at `AI_BASE_URL` is listed, ranked
  generically (`rank.ts`), and the first few candidates probed (`catalog.ts`); the selection is shared through
  Upstash (`models.ts`) and refreshed daily by Vercel Cron (`vercel.json`). `AI_MODEL` / `AI_SUGGESTION_MODEL` pin.
